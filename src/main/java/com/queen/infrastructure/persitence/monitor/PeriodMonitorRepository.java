...
public interface PeriodMonitorRepository extends ReactiveCrudRepository<PeriodMonitor, String> {
	@Query("SELECT pm.* FROM period_monitor pm INNER JOIN monitor_type mt ON pm.monitorTypeId = mt.id WHERE pm.userId = :userId AND pm.monitorTypeId = :monitorTypeId GROUP BY pm.id ORDER BY pm.created_at LIMIT :#{#pageable.getPageSize()} OFFSET :#{#pageable.getOffset()}")
